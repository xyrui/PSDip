...
parser.add_argument('-sensor', default="WV2", type=str) # sensor name, e.g., WV2, WV3, QB
...
sensor = args.sensor
rtype = 'reduced'  # reduced resolution
...
channel = {'WV2':8, 'WV3':8, 'QB':4}

# bands are used for generating the pseudo color images of intermediate products
bands = {'WV2':torch.tensor([7,1,0], dtype=torch.int), \
         'WV3':torch.tensor([7,1,0], dtype=torch.int),\
         'QB':torch.tensor([1,3,0], dtype=torch.int)}
...
for dindex in range(20):
    ...
    data = loadmat("/blabla/"+sensor+"/reduced/Test(HxWxC)_"+sensor.lower()+"_data"+str(dindex+1)+".mat") # please change the this line according to your own data dir
    gt = np.float32(data["gt"]/2047.) # HRMS
    Tgt = torch.from_numpy(gt).permute(2,0,1).unsqueeze(0).to(device)
    ...
    psnr = my_psnr(lrms_up, gt)
    print(f"Start  PSNR: {psnr:.2f}")
    ...
    for i in range(3000): 
        ...
        if saveimage:
            if (i+1)%10 == 0 and (i+1)<200:
                ...
                print(f"Iter {i+1:d}  PSNR: {psnr:.2f} loss: {loss.item():.4e}")
    ...
    # \tilde{G}
    Gn = Gnet(Tgt.detach(), Tpan.detach()).detach().squeeze(0).permute(1,2,0).cpu().numpy()
    ...
    savemat(join(savedir, str(dindex+1) + ".mat"), {'I_myDIP': X.squeeze(0).permute(1,2,0).cpu().numpy(), 'G': G.squeeze(0).permute(1,2,0).cpu().numpy(), \
                                                    'G_loss':G_loss, 'all_loss':all_loss, 'psnr_list':psnr_list, \
                                                    'Gn':Gn, 'G_init':G_init, 'Time':our_time})
